...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'G':
		...
		break;
	case 'O':
		m_glRenderer.m_fill = !m_glRenderer.m_fill, Invalidate();
		break;
	case 'N':
		m_glRenderer.m_normalsActive = !m_glRenderer.m_normalsActive, Invalidate();
		break;
	case '1':
		m_glRenderer.m_redLightActive = !m_glRenderer.m_redLightActive, Invalidate();
		break;
	case '2':
		m_glRenderer.m_greenLightActive = !m_glRenderer.m_greenLightActive, Invalidate();
		break;
	case '3':
		m_glRenderer.m_blueLightActive = !m_glRenderer.m_blueLightActive, Invalidate();
		break;
	default:
		break;
	}
...
{
	if (m_sceneRotating)
		m_glRenderer.ChangeView(0., (DOUBLE)point.y - m_oldMousePosition.y, (DOUBLE)point.x - m_oldMousePosition.x), Invalidate();
