...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'O':
		...
		break;
	case VK_ADD:
		m_glRenderer.ChangeView(-m_glRenderer.DELTA_RADIUS, 0., 0.), Invalidate();
		break;
	case VK_SUBTRACT:
		m_glRenderer.ChangeView(m_glRenderer.DELTA_RADIUS, 0., 0.), Invalidate();
		break;
	case VK_LEFT:
		m_glRenderer.ChangeView(0., 0., m_glRenderer.DELTA_ANGLE), Invalidate();
		break;
	case VK_RIGHT:
		m_glRenderer.ChangeView(0., 0., -m_glRenderer.DELTA_ANGLE), Invalidate();
		break;
	case VK_UP:
		m_glRenderer.ChangeView(0., m_glRenderer.DELTA_ANGLE, 0.), Invalidate();
		break;
	case VK_DOWN:
		m_glRenderer.ChangeView(0., -m_glRenderer.DELTA_ANGLE, 0.), Invalidate();
		break;
