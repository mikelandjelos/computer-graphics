...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'O':
		...
		break;
	case VK_LEFT:
		m_glRenderer.m_rotatingPartAngle += 5., Invalidate();
		break;
	case VK_RIGHT:
		m_glRenderer.m_rotatingPartAngle -= 5., Invalidate();
		break;
...
{
	if (m_rotating && m_oldX != -1 && m_oldY != -1)
		m_glRenderer.ChangeView(0.f, (double)point.y - m_oldY, (double)point.x - m_oldX), Invalidate();
